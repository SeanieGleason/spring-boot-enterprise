...
import com.enterprise.build.classpath.CheckClasspathForProhibitedDependencies;
import com.enterprise.build.optional.OptionalDependenciesPlugin;
import com.enterprise.build.testing.TestFailuresPlugin;
import com.enterprise.build.toolchain.ToolchainPlugin;
...
public class JavaConventions {
	...
	private void configureSpringJavaFormat(Project project) {
		...
		checkstyle.setToolVersion(project.getProperties().get("checkstyleToolVersion").toString());
	...
	private void configureDependencyManagement(Project project) {
		...
		configurations.matching(
				(c) -> c.getName().endsWith("Classpath") || c.getName().toLowerCase().endsWith("annotationprocessor"))
				.all((c) -> c.extendsFrom(dependencyManagement));

		Dependency springBootDependencies = project.getDependencies()
				.enforcedPlatform(project.getDependencies().project(Collections.singletonMap("path",
						":spring-boot-enterprise-project:spring-boot-enterprise-dependencies")));
